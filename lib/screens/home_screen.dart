...
import 'package:myfinalpro/screens/start_test_screen.dart' as monthly_test_intro_screen; // Timetest
// --- نهاية التغيير ---
...
class _HomeScreenState extends State<HomeScreen> {
      ...
      builder: (BuildContext dialogContext) {
        return AlertDialog(
          ...
          content: Text(
            message,
            textAlign: TextAlign.center,
            style: const TextStyle(fontFamily: 'Cairo', fontSize: 16.0, height: 1.5),
          ),
  ...
  Future<void> _startMonthlyTest() async {
    ...
    try {
      ...
      final ApiService apiService = ApiService(); // إذا لم يكن لديك كائن ApiService عام
      final result = await apiService.getMonthlyTestDetails(); // لا تحتاج jwtToken هنا إذا كانت الدالة تتعامل معه داخليًا

      if (!mounted) return;

      result.fold(
        (failure) {
          debugPrint("HomeScreen: Failed to fetch monthly test - ${failure.message}");
          String userMessage;
          // --- *** تعديل هنا لتخصيص رسالة الخطأ *** ---
          if (failure is ServerFailure && (
              failure.message.toLowerCase().contains("لم يتم إنهاء جلسة التدريب بعد") ||
              failure.message.toLowerCase().contains("لا يمكن بدء اختبار الشهر") ||
              failure.message.toLowerCase().contains("cannot start monthly test yet") // إضافة تحقق بالإنجليزية
            )
          ) {
            userMessage = "لا يوجد اختبار شهر متاح الآن. يرجى إكمال جلسات التدريب الخاصة بك أولاً.";
          } else if (failure is NotFoundFailure) {
             userMessage = "لا يوجد تقييم شهري متاح حاليًا. يرجى إكمال المزيد من الجلسات التدريبية أولاً.";
          } else if (failure is ServerFailure && failure.message.contains("Unauthorized")) {
             userMessage = "انتهت صلاحية الدخول. يرجى تسجيل الدخول مرة أخرى.";
          } else if (failure is ServerFailure) {
            userMessage = "حدث خطأ من الخادم أثناء محاولة تحميل الاختبار الشهري. حاول مرة أخرى لاحقًا.";
          } else {
            userMessage = "فشل تحميل بيانات التقييم الشهري (${failure.message}). حاول مرة أخرى أو أكمل جلساتك.";
          }
          // --- *** نهاية التعديل هنا *** ---
        ...
        (monthlyTestResponse) {
          ...
          String unavailabilityMessage = "لا يوجد تقييم شهري متاح حاليًا. يرجى إكمال المزيد من الجلسات التدريبية أولاً.";

          // --- *** تعديل هنا لتخصيص رسالة عدم التوفر *** ---
          if (monthlyTestResponse.messageFromApi != null) {
            String apiMsgLower = monthlyTestResponse.messageFromApi!.toLowerCase();
            if (apiMsgLower.contains("لم يتم إنهاء جلسة التدريب بعد") ||
                apiMsgLower.contains("لا يمكن بدء اختبار الشهر") ||
                apiMsgLower.contains("cannot start monthly test yet") // إضافة تحقق بالإنجليزية
            ) {
              isTestAvailable = false;
              unavailabilityMessage = "لا يوجد اختبار شهر متاح الآن. يرجى إكمال جلسات التدريب الخاصة بك أولاً.";
            } else if (apiMsgLower.contains("not available") ||
                       apiMsgLower.contains("غير متاح") ||
                       apiMsgLower.contains("لا يوجد اختبار")) {
              isTestAvailable = false;
              unavailabilityMessage = "${monthlyTestResponse.messageFromApi}. يرجى إكمال المزيد من الجلسات التدريبية.";
            } else if (monthlyTestResponse.details.isNotEmpty || monthlyTestResponse.newDetails.isNotEmpty) {
          ...
          if (isTestAvailable) {
            Navigator.push(
              context,
              MaterialPageRoute(builder: (context) => const monthly_test_intro_screen.Timetest())
            ).then((_) => _loadTokenAndInitialData());
          } else {
    ...
    return Directionality(
      ...
      child: Scaffold(
        ...
        appBar: AppBar(
          ...
          actions: [
            Padding(
                padding: const EdgeInsets.only(left: pageHorizontalPadding, top: 8.0,),
                child: Builder(
        ...
        body: showAppLoadingIndicator
            ...
            : RefreshIndicator(
                ...
                child: ListView(
                  ...
                  padding: const EdgeInsets.symmetric(horizontal: pageHorizontalPadding, vertical:16.0),
                  children: [
                    const Center(
                        child: Text('مرحبا !',
                            style: TextStyle(
                                        ...
                                        Expanded(
                                            ...
                                            child: Column(
                                                ...
                                                children: [
                                                  ...
                                                  const SizedBox(height: 16.0),
                                                  ElevatedButton(
                                                      onPressed: (isSessionAvailable && _nextSessionData != null && _nextSessionData!.typeId != 99 && !_isFetchingTestGroup && !_isFetchingMonthlyTest)
                                                          ? _startSession
                                                          : null,
                                                      ...
                                                      child: Text('لنبدأ',
                                                          style: TextStyle(
                                                              color: (isSessionAvailable && _nextSessionData != null && _nextSessionData!.typeId != 99 && !_isFetchingTestGroup && !_isFetchingMonthlyTest)
                                                                  ? primaryBlue
                        ...
                        children: [
                          Expanded(
                              child: _buildAssessmentCard(
                                  ...
                                  onPressed: _isFetchingMonthlyTest || _isFetchingTestGroup
                                      ? null
                                      : _startMonthlyTest,
